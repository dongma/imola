...
// Comparator 用于比较两个对象的大小，src < dst,返回-1. src=dst，返回0, src > dst返回1
type Comparator[T any] func(src T, dst T) int

// PriorityQueue 是一个基于小顶堆的优先级队列，当capacity为0时，为无界队列，容量切片会动态扩缩容
// 当capacity不为0时，为有界队列，初始化就固定容量，不会扩缩容
type PriorityQueue[T any] struct {
	// 用于比较前一个元素是否小于后一个元素
	compare Comparator[T]
	// 队列容量
	capacity int
	// 队列中的元素，为便于计算父子节点的index, 0位置为空, 根节点从1开始
	data []T
}
...
// NewPriorityQueue 创建优先级队列，capacity <= 0时，为无界队列
