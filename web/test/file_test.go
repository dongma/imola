...
func TestFileUploader_Handle(t *testing.T) {
	...
	server.GET("/upload_page", func(ctx *web.Context) {
...
`)
	...
	server.POST("/download", (&web.FileUploader{
		FileField: "myFile",
...
func TestFileDownloader_Handler(t *testing.T) {
	server := web.NewHTTPServer()
	server.GET("/download", (&web.FileDownloader{
...
func TestStaticResource_Handler(t *testing.T) {
	server := web.NewHTTPServer()
	handler := web.NewStaticResourceHandler("./testdata/img", "jpeg")
